...
public class IabHelper
{
	...
	void flagStartAsync(String operation)
	{
		if (mAsyncInProgress)
		{
			AVUtility.MakeDialogBox("Restoring Purchases", "A restore purchases operation is already in progress. Any purchases will automatically be given once restored.");
		} else {
	...
	void HandleIAPError(int response)
	{
		if(response == BILLING_RESPONSE_RESULT_ITEM_ALREADY_OWNED){
			AVUtility.MakeDialogBox("In-App Billing", "You already own this item! Press Restore to re-download your content if you have not already received it!");
		}
		if(response == BILLING_RESPONSE_RESULT_ITEM_UNAVAILABLE){
			AVUtility.MakeDialogBox("In-App Billing", "The requested item is unavailable at this time.");
		}
	}
}