...
namespace AceViral {
	public class AVFacebook { // Do not refactor this to 'Facebook', please. This class refers to external Facebook object!!!
		...
		public void Initialize(){
			...
			m_HasUserSignedInPreviously =Prefs.GetBool (prefkey_FirstSignIn, false);
		}
		...
		private void CheckForFirstSignIn(){
			if (!m_HasUserSignedInPreviously) {
				Prefs.SetBool (prefkey_FirstSignIn, true);
		...
		public bool HasPostedToday(){
			try{
				return (DateTime.Now.Date - DateTime.Parse (Prefs.GetString (prefkey_LastPostTime)).Date).Days <= 0;
			} catch (Exception e){
		...
		public bool HasInvitedFriendsToday(){
			try{
				return (DateTime.Now.Date - DateTime.Parse (Prefs.GetString (prefkey_LastInviteTime)).Date).Days <= 0;
			} catch (Exception e){
				AVDebug.DumpException (e);
				return false;
			}
		...
		private void OnPostDialogRequestFinished(FBResult result){
			if (string.IsNullOrEmpty (result.Error)) {
				if (result.Text.Contains ("cancelled") && result.Text.Contains ("true")) {
					...
					Prefs.SetString (prefkey_LastPostTime, DateTime.Now.Date.ToString());
				}
			} else {
		...
		private void OnInviteDialogRequestFinished(FBResult result){
			...
			if (string.IsNullOrEmpty (result.Error)) {
				if (result.Text.Contains ("cancelled") && result.Text.Contains ("true")) {
					...
					Prefs.SetString (prefkey_LastInviteTime, DateTime.Now.Date.ToString());
				}
			} else {
				Debug.LogError ("Error while facebooked: " + result.Error);
			}
		...
		private void LoadFriendsFromPrefs()
		{
			...
			string rawFriendData = Prefs.GetString(prefkey_AllUserData);
		...
		private void SaveFriendsToPrefs()
		{
			...
			Prefs.SetString(prefkey_AllUserData, sb.ToString());
			Prefs.Save();
