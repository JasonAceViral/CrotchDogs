...
public class AVHouseAdManager : MonoBehaviour
{
    ...
    {
        public const string
			iOS = AVAppConstants.iOSHouseAdLink,
			Android = AVAppConstants.AndroidHouseAdLink,
			Amazon = AVAppConstants.AmazonHouseAdLink,
            Mac = "",
			Windows8 = AVAppConstants.WindowsHouseAdLink,
		WindowsPhone8 = AVAppConstants.WP8HouseAdLink,
            WebPlayer = "",
            Linux = "";
    ...
    private static AVHouseAdManager m_Manager;
    // Always use the ad manager through AVHouseAdManager.Manager
    public static AVHouseAdManager Manager
    {
        get
        {
            if (m_Manager == null)
            {
                ...
                m_Manager = new GameObject().AddComponent<AVHouseAdManager>();
    ...
    private List<AVHouseAd> m_AdvertList;
    private string m_XmlSrc = string.Empty;
    private bool m_IsDownloadingNewXML = false;
    private bool m_IsDownloadingImage = false;
    private string cachePath = null;
    private AVHouseAd m_CurrentAdvert;
    private AVHouseAd m_CurrentFullscreenAdvert;
    private List<AVHouseAdSlot> m_AdSlots;
    ...
    private void LoadXmlFromCache()
    {
        ...
        else
        {
            ...
            {
                AVHouseAd advert = m_AdvertList[i];
    ...
    public AVHouseAd GetAdFromSlot(string slot)
    {
    ...
    private AVHouseAdSlot GetAdsWithSlotID(string slot)
    {
        AVHouseAdSlot newSlot = new AVHouseAdSlot();
        newSlot.Name = slot;
        if (m_AdSlots != null)
        {
            foreach (AVHouseAdSlot adSlot in m_AdSlots)
            {
                if (adSlot.Name == slot)
                    return adSlot;
            }
            ...
            if (m_AdvertList != null)
            {
                foreach (AVHouseAd advert in m_AdvertList)
                {
        ...
        else
        {
            ...
            m_AdSlots = new List<AVHouseAdSlot>();
        }
        return newSlot;
    }

    public AVHouseAd GetAdvertAtSlot(string slot)
    {
        return GetAdvertAtSlot(slot, false);
    }

    public AVHouseAd GetAdvertAtSlot(string slot, bool allowNullImage)
    {
        if (m_AdvertList != null)
        {
            foreach (AVHouseAd ad in m_AdvertList)
            {
    ...
    private IEnumerator DownloadImageForAdvert(AVHouseAd advert)
    {
    ...
    private IEnumerator DownloadImageToAdvert(AVHouseAd advert, string savePath)
    {
    ...
    private void ParseXmlFile()
    {
        ...
        while (xmlReader.Read())
        {
            if (xmlReader.NodeType == XmlNodeType.Element)
            {
                ...
                {
                    AVHouseAd newAd = GetAdFromCurrentNode(xmlReader);
                    bool hasReplaced = false;
                    if (m_AdvertList == null)
                    {
                        m_AdvertList = new List<AVHouseAd>();
                    }
                    for (int i = 0; i < m_AdvertList.Count; i++)
                    {
    ...
    private AVHouseAd GetAdFromCurrentNode(XmlReader reader)
    {

        AVHouseAd newAd = new AVHouseAd();
