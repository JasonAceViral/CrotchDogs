...
package com.aceviral.utility.inapp;
...
import com.aceviral.utility.inapp.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection
{
	...
	abstract class BillingRequest
	{
		...
		{
			AVUtility.DebugOut(TAG, getClass().getSimpleName());
			if (mService != null)
			{
				try
				{
					mRequestId = run();
					AVUtility.DebugOut(TAG, "request id: " + mRequestId);
					if (mRequestId >= 0)
					{
		...
		protected void logResponseCode(String method, Bundle response)
		{
			...
			AVUtility.DebugOut(TAG, method + " received " + responseCode.toString());
		}
	}
	...
	class CheckBillingSupported extends BillingRequest
	{
		...
		protected long run() throws RemoteException
		{
			...
			int responseCode = response
					.getInt(Consts.BILLING_RESPONSE_RESPONSE_CODE);
			Log.v(TAG, "CheckBillingSupported response code: " + ResponseCode.valueOf(responseCode));
	...
	public class RequestPurchase extends BillingRequest
	{
		...
		protected void responseCodeReceived(ResponseCode responseCode)
		{
			AVUtility.DebugOut("BillingService: RequestPurchase: reponseCodeReceived", "Got response code: " + responseCode.name());
	...
	public void handleCommand(Intent intent, int startId)
	{
		if (intent != null && intent.getAction() != null)
		{
			...
			AVUtility.DebugOut(TAG, "handleCommand() action: " + action);
			...
			} else if (Consts.ACTION_PURCHASE_STATE_CHANGED.equals(action))
			{
				String signedData = intent
						.getStringExtra(Consts.INAPP_SIGNED_DATA);
				String signature = intent
						.getStringExtra(Consts.INAPP_SIGNATURE);
	...
	private boolean bindToMarketBillingService()
	{
		try
		{
			AVUtility.DebugOut(TAG, "binding to Market billing service");
	...
	{
		AVUtility.DebugOut("BillingService: restoreTransactions()", "Restoring transactions - Requests: " + ++m_RestoreTransactionsRequests);
		return new RestoreTransactions().runRequest();
	}
	public int m_RestoreTransactionsRequests = 0;
	...
	private void checkResponseCode(long requestId, ResponseCode responseCode)
	{
		...
		if (request != null)
		{
			AVUtility.DebugOut(TAG, request.getClass().getSimpleName() + ": " + responseCode);
	...
	private void runPendingRequests()
	{
		...
		if (maxStartId >= 0)
		{
			AVUtility.DebugOut(TAG, "stopping service, startId: " + maxStartId);
			stopSelf(maxStartId);
		}
	}
	...
	{
		AVUtility.DebugOut(TAG, "Billing service connected");
